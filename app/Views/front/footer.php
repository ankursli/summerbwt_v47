	...
	<div id="cookieChoiceInfo" style="display: none; position: fixed; width: 100%; background-color: #00649a; margin: 0px; left: 0px; bottom: 0px; padding: 4px; z-index: 1000; text-align: center;color:#fff;">
...
$(document).ready(function () {
    // Allow only letters and spaces for #city_register and #vilep
    ...
    // Allow only digits for #postalcodep and ensure it is exactly 5 digits
    ...
    // Enforce exact length of 5 digits on blur for #postalcodep
    $("#postalcodep").on("blur", function () {
        var postalCode = $(this).val();
        if (postalCode.length !== 5) {
            alert("Postal code must be exactly 5 digits long.");
            setTimeout(() => { $(this).focus(); }, 1); // Delay to ensure alert closes properly
        }
    });

    // Allow only digits for .allow-numeric elements
    ...
    // Allow only digits for #mobNum and ensure it is exactly 10 digits
    ...
    // Enforce exact length of 10 digits on blur for #mobNum
		...
		if($_SESSION['site_lang']=='english'){						
			...
			<script>
				$(document).ready(function() {
					...
					var condition = base_url+'page/Conditions_de_participation';
					var reules = base_url+'page/reglement_en';
					var privacy_policy = base_url+'page/privacy_policy';
					var legal_notice = base_url+'page/legal-notices';
					var cookie_link = base_url+'page/policy-cookies';
					
					var conditionfr = base_url+'page/modalites';
					var reulesfr = base_url+'page/reglement';
					var privacy_policyfr = base_url+'page/politique-de-confidentialite';
					var legal_noticefr = base_url+'page/mentions-legales';
					var cookie_linkfr = base_url+'page/politique-cookies';
...
$(document).ready(function() {
	if(localStorage.getItem('summerbwt_cookie_accept') !== 'accepted'){
	...
	$("#cookieChoiceDismiss").click(function(e){
		e.preventDefault();
		localStorage.setItem('summerbwt_cookie_accept', 'accepted');
		$('#cookieChoiceInfo').fadeOut();
		$('body').removeClass('cookieclass');
		$('#footerr').removeClass('cookieclass');
	});
	$("#cookieChoiceDismissIcon").click(function(e){
		e.preventDefault();
		localStorage.setItem('summerbwt_cookie_accept', 'rejected');
		$('#cookieChoiceInfo').fadeOut();
		$('body').removeClass('cookieclass');
		$('#footerr').removeClass('cookieclass');
	});
...
$('#selectrobotcountry').change(function(){ 
                ...
                $.ajax({
                    url : "<?php echo base_url('front/get_country_robot_store_handle');?>",
                    method : "POST",
	...
	var storcount=$('#sotrycountry').val();
                ...
                $.ajax({
                    ...
                    success: function(response){
						...
						$.each(response,function(index,data){
						    if (data['id'] === 'AUTRE' || data['store_name'] === 'AUTRE') {
					...
					fileReader.onload = (function(e) {
					  ...
					  if(!empty($_SESSION['site_lang'])){
								...
								if(extentionmane == 'pdf'){
									$("<span class=\"pip\">" +
									"<img class=\"imageThumb\" src=\"assets/image/pdf.png\" title=\"" + data + "\"/>" +
