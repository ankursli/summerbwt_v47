...
class Front extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model('Mdl_Storerobot');
...
<?php
	...
	public function proof_of_purchase()
	{
		...
		if (!empty($_SESSION['front_user'])) {
			...
			$getstores = $this->Mdl_Storerobot->GetRecordUsers();

			// $getrobotstores = $this->Mdl_Storerobot->GetRecordUsers();

			$getrobots = $this->Mdl_Robot->GetRecordUsers();

			$anothergetstores = $this->Mdl_Storerobot->GetRecordUsers(array('store_handle' => 0));

			$getcoupons = $this->Mdl_Coupon->GetRecordUsers();

			// print_r($getrobotstores);exit;
			if (!empty($getproofs)) {
	...
	{
		// print_r($_SESSION['front_user']);exit;
		if (!empty($_SESSION['front_user'])) {
			// print_r($_SESSION['front_user']);exit;
			...
			if (!preg_match('/^\d{5}$/', $postcode)) {
				$this->session->set_flashdata('error', 'Please update your postal code from your profile, It should be 5 Digits.');
			...
			if ($ispost == 'POST') {
				...
				$rbtid = $this->input->post('robot_id');
				$storename = $this->input->post('store_id');
				// set validation rules
				$siteLang = $this->session->userdata('site_lang');

				if ($siteLang == 'english') {
					$this->form_validation->set_rules('robot_id', "Modal", 'trim|required|callback_select_validate2');
					...
					$this->form_validation->set_rules('robot_id', 'Modal', 'required|callback_select_validate2', array('required' => 'Le champ robot est obligatoire.'));
				...
				$robot_code = $this->Mdl_Robot->GetRecordUsers(array("id" => $robot_id));
				...
				$store_code = $this->Mdl_Storerobot->GetRecordUsers(array("id" => $store_id));
				...
				if ($this->form_validation->run() === FALSE) {
					...
					if (!empty($_FILES['upload_proof']['name']) || !empty($uplodehidenfile)) {
						...
						if ($this->Mdl_Proof->insert($insert)) {

							$datef = str_replace('/', '-', $date_of_purchase);
							$new_date = date('Y-m-d', strtotime($datef));
							...
							$httpCode = $resultArray['http_code'];
							$lastInsertId = $this->db->insert_id();

							$this->db->set('api_status', $httpCode);
							$this->db->set('api_post_data', json_encode($parameters));
							$this->db->set('api_get_data', $result);
							$this->db->where('purchase_id', $lastInsertId);
							$this->db->update('proof_of_purchase');
							...
							if ($httpCode !== 200) {
								...
								if ($siteLang == 'english') {
									$this->session->set_flashdata('error', 'Your data is not ok, please check!');
								} else {
									$this->session->set_flashdata('error', 'Vos données ne sont pas correctes, veuillez vérifier !');
								}
	...
	// 								// curl_close($curl);
	// 								if($langcurrent == "english"){
	// 									$user_get_templates_html=$this->Mdl_Template->GetRecordUsers(array('id'=>32));
	// 								}
	// 								else{

	// 									$user_get_templates_html=$this->Mdl_Template->GetRecordUsers(array('id'=>7));
	// 								}
	// 								$user_html = $user_get_templates_html[0]['template'];	
	// 								$settings = $this->Mdl_Settings->GetRecord();
					...
					if (!empty($_FILES['upload_draw']['name']) || !empty($uplodehidenfile)) {
						...
						if ($result) {
							...
							try {
								...
								// print_r('exitt');exit;
								if ($siteLang == 'english') {
									$this->session->set_flashdata('error', 'Error while creating Draw!');
								} else {
									$this->session->set_flashdata('error', 'Erreur lors de la création de Draw!');
								}
								$getrefunds = $this->Mdl_Refund->GetRecordUsers(array('refund.user_id' => $_SESSION['front_user']['id']));
								$getstores = $this->Mdl_Store->GetRecordUsers();
								$getcoupons = $this->Mdl_Coupon->GetRecordUsers();
		...
		if (!empty($_SESSION['front_user'])) {
			...
			if ($ispost == 'POST') {
				...
				if ($siteLang == 'english') {
					...
					$this->form_validation->set_rules('postcode', 'Postcode', 'required|regex_match[/^[0-9]{5}$/]', array('required' => 'Le code postal est obligatoire.', 'regex_match' => 'Veuillez ajouter le code postal valide comme 69001 (5 chiffres)'));
					$this->form_validation->set_rules('city', 'City', 'required', array('required' => 'Le champ Ville est obligatoire.'));
					$this->form_validation->set_rules('country', 'Country', 'required', array('required' => 'Le champ Pays est obligatoire.'));
										...
										[
											...
											'To' => [
												...
												[
													...
													'Email' => 'service.client-bwt@take-off.fr',
	...
	public function cron_proof()
	{
		...
		if (!empty($proofs)) {
			...
			foreach ($proofs as $proof) {
				...
				$siteLang = $this->session->userdata('site_lang');
				if ($siteLang == 'english') {
					$get_templates = $this->Mdl_Template->GetRecordUsers(array('id' => 13));
				} else {
					$get_templates = $this->Mdl_Template->GetRecordUsers(array('id' => 13));
				}
				...
				//	$response = $mj->post(Resources::$Email, ['body' => $body]);
				$config = $this->config->item('email'); // load from email.php
	...
	public function get_country_store_handle()
	{
		...
		if (empty($country_id) || empty($storeid)) {
			log_message('error', 'Invalid input data: country_id or storeid is missing');
		...
		try {
			if ($storeid == 3) {
				$data = $this->Mdl_Storerobot->get_country_store_handle($country_id, $storeid);
			} else {
			...
			// Sorting data by store_name safely
			$price = [];
			foreach ($data as $key => $row) {
				$store_name = $row['store_name'] ?? 'Unknown Store';
				$price[$store_name] = $store_name;
			}
	...
	public function get_country_robot_store_handle()
	{
		...
		$data = $this->Mdl_Storerobot->get_country_robot_store_handle($country_id, $storeid);
		...
		foreach ($data as $key => $row) {
			if (!isset($row['store_name'])) {
				echo json_encode(['error' => 'store_name key missing', 'row' => $row]);
				return;
			}
			$price[$key] = $row['store_name']; // Use index instead of store_name as key
		}
	...
	public function sendMail()
	{
		...
		try {
			...
			$mail->SMTPAuth   = true;
			$mail->Username   = 'bwt22';
			$mail->Password   = 'UPWKng3ZrMtql5tB';
			// $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
			$mail->SMTPSecure = 'tls';
			$mail->Port       = 587; // or 8025, 587, 25

			// Email content settings
			$mail->setFrom('operations.bwt@phare-west.fr', 'Your Name');
			$mail->addAddress('john2012007@gmail.com', 'Recipient Name');
			$mail->isHTML(true);
			$mail->Subject = 'Test Email from PHPMailer';
			$mail->Body    = '<h1>This is a test email</h1><p>Sent using PHPMailer via SMTP2GO</p>';
			$mail->CharSet = 'UTF-8';
