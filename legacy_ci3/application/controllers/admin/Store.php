...
class Store extends CI_Controller {
	...
	public function Addstore()
	{
		if(empty($_SESSION['admin_user'])){
			...
			$data=array('success'=>$this->session->flashdata('success'),
					'error'=>$this->session->flashdata('error'),
					'main_content'=>'admin/store/create',
					'countries'=>$countries
					);
			$this->load->view('admin/front',$data);
		}
	...
	public function edit($id=null)
	{
		if(empty($_SESSION['admin_user'])){
			...
			if(!empty($id)){
				...
				$stores=$this->Mdl_Store->GetRecordUsers(array('id'=>$id));
				$data=array('success'=>$this->session->flashdata('success'),
						'error'=>$this->session->flashdata('error'),
						'main_content'=>'admin/store/edit',
						'stores'=>$stores,
						'countries'=>$countries
						);
				$this->load->view('admin/front',$data);
			...
			if(!empty($id)){
				...
				if($ispost=='POST'){
					...
					if($this->form_validation->run() === FALSE){
						...
						if($this->Mdl_Store->update($update,array('id'=>$id))){
							if($siteLang=='english'){
	...
	public function remove($id=null)
	{
		...
		if(empty($_SESSION['admin_user'])){
			...
			if($ispost == 'GET')
			{
				$remove=$this->Mdl_Store->remove($id);
				if(empty($remove))
				{
					if($siteLang=='english'){
