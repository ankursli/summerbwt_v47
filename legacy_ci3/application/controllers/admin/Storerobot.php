...
class StoreRobot extends CI_Controller {
	...
	public function Addstore()
	{
		if(empty($_SESSION['admin_user'])){
			...
			$data=array('success'=>$this->session->flashdata('success'),
					'error'=>$this->session->flashdata('error'),
					'main_content'=>'admin/storerobot/create',
					'countries'=>$countries
					);
			$this->load->view('admin/front',$data);
		}
	...
	public function edit($id=null)
	{
		if(empty($_SESSION['admin_user'])){
			...
			if(!empty($id)){
				...
				$stores=$this->Mdl_Storerobot->GetRecordUsers(array('id'=>$id));
				$data=array('success'=>$this->session->flashdata('success'),
						'error'=>$this->session->flashdata('error'),
						'main_content'=>'admin/storerobot/edit',
						'stores'=>$stores,
						'countries'=>$countries
						);
				$this->load->view('admin/front',$data);
			...
			if(!empty($id)){
				...
				if($ispost=='POST'){
					...
					if($this->form_validation->run() === FALSE){
						...
						if($this->Mdl_Storerobot->update($update,array('id'=>$id))){
							if($siteLang=='english'){
	...
	public function remove($id=null)
	{
		...
		if(empty($_SESSION['admin_user'])){
			...
			if($ispost == 'GET')
			{
				$remove=$this->Mdl_Storerobot->remove($id);
				if(empty($remove))
				{
					if($siteLang=='english'){
		...
		if(!empty($_SESSION['admin_user'])){
			...
			if(!empty($_FILES['storefile']['name']) && $ext == 'csv'){
				...
				while($csv_line = fgetcsv($fp,1024))
				{
					...
					$id = $this->Mdl_Storerobot->insert($data);
				}
