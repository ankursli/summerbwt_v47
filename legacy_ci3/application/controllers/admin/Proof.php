...
class Proof extends CI_Controller {
	...
	public function edit($id=null)
	{
		if(empty($_SESSION['admin_user'])){
			...
			if(!empty($id)){
				$proofs=$this->Mdl_Proof->GetProofofcoupon(array('purchase_id'=>$id));
	...
	public function export_proof() {
		// Initialize CSV headers with BOM for UTF-8 compatibility
		$second_export = chr(0xEF) . chr(0xBB) . chr(0xBF) . "Id,Username,Full Name,Phone,Robot,Stores,Another Stores,Address,Postcode,City,Upload Proof,Proof Date,Client Type,IBAN,BIC,Robot Serial No,Status\n";
		...
		foreach ($proofs as $proof) {
			// Get store name
			$name = $proof['store_id'] != 0 ? ($this->Mdl_Storerobot->GetRecordUsers(array('id' => $proof['store_id']))[0]['store_name'] ?? '') : '';
	
			// Check for 'AUTRE' store and set additional fields
			if ($name == 'AUTRE') {
			...
			$robot_name = $robot[0]['robot_name'] ?? '';
	
			// Set proof image URL
			$img = base_url('upload') . '/op3/' . $proof['upload_proof'];
	
			// Set status
			$status = $proof['status'] == 1 ? 'Approved' : ($proof['status'] == 2 ? 'Rejected' : 'Pending');
	
			// Set client type
			$clienttype = $proof['clienttype'] == 'particulier' ? 'Particulier' : ($proof['clienttype'] == 'pro' ? 'Professional' : 'NULL');
	
			// Set other proof fields
			$iban = $proof['iban'];
			$bic = $proof['bic'];
	
			// Fetch user details based on email
			$user_details = $this->Mdl_User->GetRecordUsers(array('email' => $proof['email']));
			$fullname = isset($user_details[0]['firstname']) && isset($user_details[0]['lastname']) ? $user_details[0]['firstname'] . ' ' . $user_details[0]['lastname'] : '';
			$phone_number = isset($user_details[0]['phone']) ? $user_details[0]['phone'] : '';
			...
			$export_row = array(
				...
				$iban,
				$bic,
				$proof['robot_serial_no'],
				$status
			);
	
			// Join the array elements with commas to form a CSV row, with each element properly quoted
			$second_export .= '"' . implode('","', $export_row) . '"' . "\n";
		}
		...
		$csv_path = FCPATH . '/upload/robot/' . $filename;
		...
		echo file_get_contents($csv_path);
		exit;
	}
			...
			if($status==1){
				...
				if(!empty($id) && !empty($status) && !empty($coupon_list_code)){
					...
					if($status==1){
						...
						if($this->Mdl_Proof->update($update,array('purchase_id'=>$id))){
							//get coupon code details
							$get_proof=$this->Mdl_Proof->GetCouponCodeDetails($coupon_list_code);
		...
		if(empty($_SESSION['admin_user'])){
			...
			if($status==1){
				...
				if($used_limit>$usedcoupon){
					...
					if($this->Mdl_Proof->update($update,array('purchase_id'=>$id))){
						//get coupon code details
						$get_proof=$this->Mdl_Proof->GetCouponCodeDetails($store_code);
	...
	public function remove($id=null)
	{
		...
		if(empty($_SESSION['admin_user'])){
			...
			if($ispost == 'GET')
			{
				$removecoupon=$this->Mdl_Proof->remove(array('purchase_id'=>$id));
	...
	public function test(){
		$get_proof=$this->Mdl_Proof->GetCouponCodeDetails('beADFKMN');
		$today = date('Y-m-d');
		$validity_date = $get_proof['validity_date'];
		if($today<=$validity_date){
			echo 'send';
