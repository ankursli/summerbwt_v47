...
class Mdl_Proof extends CI_Model 
{
    ...
    {
		$this->db->select('proof_of_purchase.*,users.*,store.*,coupon.*');
		$this->db->from('proof_of_purchase');
		$this->db->join('users', 'proof_of_purchase.user_id = users.id');
		$this->db->join('store', 'proof_of_purchase.store_id = store.id');
		$this->db->join('coupon', 'proof_of_purchase.coupon_id = coupon.id');
		 if(!empty($conditions))
			$this->db->where($conditions);
		$query = $this->db->get();
